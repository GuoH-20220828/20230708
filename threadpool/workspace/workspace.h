...
/*******************************************************************************************
	...
	> Created Time: 2023年04月13日 星期四 16时23分01秒
 ******************************************************************************************/
#include <cassert>
#include <map>
#include <memory>
...
namespace wsp 
{
    ...
    private:
        using branch_lst = std::list<std::unique_ptr<workbranch>>;   //workbranck list
        using superv_map = std::map<const supervisor*, std::unique_ptr<supervisor>>;  //supervisor map
        using pos_t = branch_lst::iterator;   //workbranck list iterator
