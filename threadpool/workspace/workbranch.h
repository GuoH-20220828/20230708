#pragma once
/*******************************************************************************************
	> File Name: workbranch.h
	> Author: Guo Hang
	> Mail: 15091721302@163.com
	> Created Time: 2023年04月08日 星期六 18时07分24秒
 ******************************************************************************************/
#include <cstdlib>
#include <map>
#include <memory>
...
namespace wsp::details 
{
    class workbranch 
    {
        ...
        sz_t decline = 0;   //number of deleted tasks
        sz_t task_done_workers = 0;  //number of 
        bool is_waiting = false;   //wait_tasks()
        bool destructing = false;   //~workbranch() 
    ...
    public:
        ...
        /**
         * @brief async execute the task 
         * @param task runnable object (urgent)  当你有一个任务并且你希望它能尽快被执行时，你可以指定该任务的类型为`urgent`
         * @return void
         */
    ...
    private:
        ...
        void mission() 
        {
            ...
            while (true) 
            {
                ...
                {   //no task processing in tq
                    if (is_waiting)   //wait_tasks
                    {          
                        std::unique_lock<std::mutex> locker(lok);
                        task_done_workers++;  
                        task_done_cv.notify_one(); 
